...
namespace MS.ChessMaze
{
	public class CandidateMap
	{
		...
		private int cornersEachOtherCount;
		...
		public void FindPath()
		{
			...
			this.cornersEachOtherCount = CalculateCornersNearEachOther(this.cornersList);
		...
		private List<Vector3> GetListOfCorners(List<Vector3> path)
		{
			...
			for (int i = 0; i < pathWithStart.Count-2; i++)
			{
				if (pathWithStart[i + 1].x > pathWithStart[i].x || pathWithStart[i + 1].x < pathWithStart[i].x)
				{
					...
					{
						cornersPosition.Add(pathWithStart[i + 1]);
					}
				}
		...
		public MapData ReturnMapData()
		{
			return new MapData
			{
				...
				cornersNearEachOther = this.cornersEachOtherCount
			};
		}
		...
		public CandidateMap(CandidateMap candidateMap)
		{
			...
			this.cornersEachOtherCount = candidateMap.cornersEachOtherCount;
