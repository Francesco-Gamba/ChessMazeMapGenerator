...
public class MapBrain : MonoBehaviour
{
    ...
    [SerializeField, Range(.3f, 1f)]
    private double fitnessObstaclesWeight = 1;
    ...
    public void RunAlgorithm()
    {
        ...
        MapHelper.RandomlyChooseAndSetStartAndExit(grid, ref startPosition, ref exitPosition, randomStartEnd, startPositionEdge, exitPositionEdge);
    ...
    private IEnumerator GeneticAlgorithm()
    {
        ...
        Debug.Log("Best of generation " + generationNumber + " : " + bestMapThisGeneration);
    ...
    private void ShowResult()
    {
        ...
        Debug.Log("Time needed to run: " + elapsedSpan.TotalSeconds);
    }
    ...
    private void CrossOverParents(CandidateMap parent1, CandidateMap parent2, out CandidateMap child1, out CandidateMap child2)
    {
        ...
        if(Random.value<crossoverRatePercent)
        {
            ...
            for (int i = crossOverIndex; i < numBits; i++)
            {
                ...
                child1.PlaceObstacle(i, parent1.IsObstacleAt(i));
            }
        }
    }
    ...
    {
        int numberOfObstacles = mapData.obstacleArray.Where(isObstacles => isObstacles).Count();
        int score = mapData.path.Count * fitnessPathWeight + (int)(numberOfObstacles * fitnessObstaclesWeight);
